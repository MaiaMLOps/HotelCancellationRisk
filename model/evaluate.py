...
def calculate_metrics(
    ...
    sample_weight=None gives the conventional per-reservation evaluation.
    Passing pattern weights gives the sensitivity analysis in which each
    unique predictor pattern has approximately equal total influence.
    """
...
def evaluate_cv(
    ...
    """
    Evaluate one model configuration with the shared 5-fold grouped CV.

    The fixed test partition is never used here.
    """
    ...
    for fold in range(n_folds):
        ...
        # Conventional evaluation: every reservation has weight 1.
        ...
        # Sensitivity evaluation: each repeated predictor pattern
        # contributes approximately equal total weight.
    ...
    for metric in METRIC_COLUMNS:
        summary[f"cv_{metric}_mean"] = float(
            results_df[metric].mean()
        )
        summary[f"cv_{metric}_std"] = float(
            results_df[metric].std(ddof=1)
        )

    summary["cv_fit_seconds_mean"] = float(
        results_df["fit_seconds"].mean()
    )
...
def print_cv_summary(
    ...
    """Print a compact summary of one experiment."""
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="all",
        help=(
            "Model to evaluate. Use 'all' to evaluate "
